...
int main (int argc, char* argv[]) 
{
	...
	for (int x = 0, y = 0; SnakePixel (image, x, y); )
	{
		...
		if(image.Pixel(x,y) == curPix){
			curLen += 1;
		}

		else{
			runLengthHistogram[curLen] += 1;
			curLen = 1;
			totalRun += 1;
			curPix = image.Pixel(x,y);
		}
	...
	for (int i = 0; i < runLengthHistogram.size (); i++) 
	{
		...
		double temp = runLengthHistogram[i]/totalRun;
		entropy -= temp * log2(temp);
	}

	double total_bits = entropy * totalRun;
	...
	cout << "Average Bits " << avg_bits << endl;
