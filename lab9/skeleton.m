...
mse =(norm((lena-lena_blur),'fro')^2)/(512*512);
fprintf("The MSE between original and blured image is: %i\n", mse);
...
mse2 =(norm((x-xhat_hor),'fro')^2)/(512*510);
...
mse3 =(norm((x - xhat_hor_ver),'fro')^2)/(512*512)
fprintf("MSE after vert: %i\n", mse3);
...
H_invfilt = fft2(invfilt,127,127);
H_invfilt = fftshift(H_invfilt);
W_invfilt = (2*pi/127).*[0:1:127-1] - pi;
figure;mesh(W_invfilt,W_invfilt,abs(H_invfilt));
xlabel('Wx --->');
ylabel('Wy --->');
zlabel('abs(H(Wx,Wy)) --->');
...
fprintf("MSE after reconstruction: %i\n", MseReconst);
...
gauss_noise = sigman*randn(512,512);
lena_gauss_noise = lena_blur + gauss_noise;
...
fprintf("MSE after gaussian reconst: %i\n", mse_g);
